    ...
    <div className="fixed inset-0 flex items-center justify-center bg-zinc-900/60 z-50 p-4">
      <div className="bg-zinc-800/90 backdrop-blur-xl border border-zinc-700/50 text-white rounded-2xl shadow-2xl w-full max-w-md mx-4">
        <div className="p-6">
          ...
          {isExpired && hasLongDebt && (
            <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-xl p-4 mb-4">
              <div className="flex items-start gap-2">
                <AlertTriangle size={16} className="text-yellow-400 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-yellow-400 text-sm font-semibold">Jugador con 6+ meses de deuda</p>
                  <p className="text-gray-400 text-xs mt-1">
                    Si este jugador estuvo fuera del equipo y no asistió, puedes condonar meses antes de cobrar usando "Ajustar Deuda".
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={onSubmit} className="space-y-6">
            <div>
              ...
              <div className="relative">
                <span className="absolute left-4 top-1/2 transform -translate-y-1/2 text-red-400 font-bold">$</span>
                <input
                  type="number"
                  step="0.01"
                  min="0"
                  ...
                  className="w-full bg-zinc-700/50 text-white pl-8 pr-4 py-4 rounded-xl border border-zinc-600/50 focus:border-red-500 focus:ring-2 focus:ring-red-500/20 focus:outline-none transition-all duration-200 text-lg font-semibold"
                />
              </div>
              {errors.amount && (
